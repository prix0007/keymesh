...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!Array.isArray(commitments) || commitments.length === 0) {
      return NextResponse.json({ error: "Commitments array is required" }, { status: 400 });
    }

    if (commitments.length !== 3) {
      return NextResponse.json({ error: "Exactly 3 commitments required (for pieces A, B, C)" }, { status: 400 });
    }
    ...
    for (const commitment of commitments) {
      ...
      if (!Number.isInteger(commitment.blockNumber) || commitment.blockNumber <= 0) {
        return NextResponse.json({ error: "Invalid blockNumber. Must be a positive integer" }, { status: 400 });
      }

      // Validate txIndex is a non-negative integer
      if (!Number.isInteger(commitment.txIndex) || commitment.txIndex < 0) {
        return NextResponse.json({ error: "Invalid txIndex. Must be a non-negative integer" }, { status: 400 });
      }
    ...
    if (uniquePieceIds.size !== 3) {
      return NextResponse.json({ error: "Piece IDs must be unique (0, 1, 2)" }, { status: 400 });
    }
    ...
    if (existingCommitments.length > 0) {
      return NextResponse.json({ error: "User already has DA commitments. Use PUT to update." }, { status: 409 });
    }

    // Create commitments in transaction
    const createdCommitments = await prisma.$transaction(async (tx: any) => {
      const results = [];
    ...
    const formattedCommitments = createdCommitments.map((commitment: any) => ({
      id: commitment.id,
      pieceId: commitment.pieceId,
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const formattedCommitments = commitments.map((commitment: any) => ({
      id: commitment.id,
      pieceId: commitment.pieceId,
      blockNumber: commitment.blockNumber.toString(),
      txIndex: commitment.txIndex,
